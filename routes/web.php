...
Route::middleware("throttle:100,1")->group(function () {
    Route::get("/", IndexController::class)->middleware(["_referral-program", "_no-queries"])->name("public.index");
    ...
    Route::get("/persist/across/{key}/{locale}", [RedisController::class, "persistAcross"])->middleware("_stable")->name("public.persist.across");
});
...
Route::middleware(["_private", "throttle:100,1"])->group(function () {
    Route::get("/private/dashboard", DashboardController::class)->middleware(["throttle:30,1", "_no-queries"])->name("private.dashboard");
...
Route::post("/post/toggleThemeState", [RedisController::class, "toggleThemeState"])->name("post.toggleThemeState");
...
Route::post("/post/components/side-menu/logout", LogoutButtonController::class)->name("post.components.side-menu.logout");
