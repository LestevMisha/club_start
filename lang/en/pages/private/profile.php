<?php

declare(strict_types=1);

return [
    "1" => "Image settings",
    "2" => "Update image",
    "3" => "Change password",
    "4" => "Autopayment settings",
    "5" => "In this block you can cancel automatic subscription to «KLUB START»",
    "6" => "Manage Subscriptions",
    "7" => "Active",
    "8" => "Password settings",
    "9" => "Enter the number of the image you want to use from Telegram in this field and click \"Save\". The avatar will be updated automatically.",
    "11" => "Verification Status",
    "12" => "Verified",
    "13" => "Not verified",
    "14" => "Inactive",
    // "15" => "Email changes are exclusively processed through your linked Telegram account. Attempts via any other account will not be recognized by the bot.",
    "16" => "Change email",
    "17" => "Current email",
    "18" => "Reset the password",
    "19" => "You will receive a letter on your email address, follow the link and create a new password.",
    "20" => "Disable Auto-Renewal",
    "21" => "Name",
    "22" => "Linked Telegram",
    "23" => "General Information",
    // "24" => "or",
    // "25" => "In this block you can manage your subscriptions",
    "26" => "Status",
    "27" => "For the amount",
    "28" => "Date of the next write-off",
];
