    ...
    <div class="flex w100 h align space-btw mx-auto">
        ...
        <div id="dashboard-link-container" class="flex h gap fit align flex_blured login_cst mob">
            <div class="flex h align gap">
                <livewire:templates.theme-switcher />
                @if ($currentRoute !== 'main')
                    @if (auth()->user()?->uuid)
                        <a href="{{ route('dashboard') }}" class="cr-button">
            ...
            <a id="dashboard-link" href="{{ route('login') }}" class="b-text b-text_fit b-text_right b-text_v3">Личный
                кабинет</a>
        </div>
    </div>
</header>