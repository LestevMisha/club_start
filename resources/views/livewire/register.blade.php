    ...
    <div class="form-wrapper">
        ...
        <form wire:submit.default="nextStep" class="modern-form">
            ...
            <div class="flex static v w100">
                @if ($currentStep >= 0)
                <x-input attribute="name" inscription="Ваше Имя" inputDisplay="{{ $currentStep > 0 ? 'none' : 'block' }}" />
                @if ($currentStep >= 1)
                    ...
                    <x-input attribute="email" inscription="Email Адрес" inputDisplay="{{ $currentStep > 1 ? 'none' : 'block' }}" />
                    @if ($currentStep >= 2)
                        <button type="button" wire:click="previousStep" class="previous-step-button">
                            <x-svg-gradient class="previous-step-icon" svg="PreviousStepArrow" />
                        </button>
