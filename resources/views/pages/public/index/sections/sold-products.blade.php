<section class="dark:bg-black relative m-0 h-full w-full bg-white p-0 pt-24">
    <div class="min-w-96 max-w-96 relative z-10 m-auto w-full xl:max-w-[75rem]">
        <div class="flex flex-col">
            <div class="flex h-fit flex-col items-center justify-center">
                <div class="dark:text-white relative mb-4 w-fit text-center text-4xl font-bold uppercase text-black xl:text-[2.5rem]">
                    {{ __('pages/public/index.89') }}
                    <span
                        class="text-nowrap dark:before:bg-[#0d6efd] relative -z-1! uppercase before:absolute before:right-0 before:top-2/4 before:-z-10 before:block before:h-10 before:w-full before:-translate-y-1/2 before:bg-[#6fa9ff] before:p-[0.3rem] before:content-[''] before:[rotate:1deg]">
                        {{ __('pages/public/index.90') }}
                    </span>
                    {{ __('pages/public/index.91') }}
                </div>
            </div>
            <div class=" uppercase relative mb-24 text-center text-base font-light text-[#acacac] xl:text-2xl">
                {{ __('pages/public/index.92') }}
            </div>
            <!-- Swiper_v3 StWoom3s -->
            <div class="relative">
                <div class="js-swiper-v3 m-0 flex flex-col justify-between overflow-visible! px-4! pb-8 xl:flex-row xl:overflow-hidden!">
                    <div class="js-justify-arrows swiper-wrapper flex w-full flex-row justify-between [transition:all_0.225s_ease-in-out]">
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/1.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_1.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/2.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_2.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/3.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_3.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/4.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_4.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/5.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_5.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/6.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_6.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/7.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_7.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/8.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_8.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/9.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_9.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/10.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_10.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/11.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_11.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                        <div class="swiper-slide [&.swiper-slide-active]:opacity-100! w-full opacity-10 [transition:opacity_0.225s_ease-in-out]! xl:opacity-100">
                            <video class="mx-auto block h-[30rem] w-72 rounded-2xl border-none object-cover" muted controls preload="none"
                                poster="{{ URL::asset('assets/images/min-jpeg/sold-products-posters/12.jpeg') }}">
                                <source src="{{ URL::asset('assets/videos/feedbacks/feedback_12.mp4') }}" type="video/mp4">
                            </video>
                        </div>
                    </div>
                    <div class="swiper-arrow swiper-button-next"></div>
                    <div class="swiper-arrow swiper-button-prev"></div>
                    <div class="swiper-pagination"></div>
                </div>
            </div>
        </div>
    </div>
</section>