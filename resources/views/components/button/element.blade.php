<xbutton class="component group cursor-pointer" data-uid="{{ $uid }}" @if ($attribute) data-attribute="{{ $attribute }}" @endif>
    @if ($url)
        <a target="_blank" href="{{ $url }}"
            class="dark:text-white dark:bg-[#80808026] dark:hover:text-[#ffffff80] relative flex h-16 w-full min-w-[10em] flex-row items-center justify-between gap-4 rounded-lg bg-[#ecf1f7] px-[1em] font-medium text-black [transition:all_.225s_ease-in-out] hover:bg-[#8080801a]">
            {{ $title }}
