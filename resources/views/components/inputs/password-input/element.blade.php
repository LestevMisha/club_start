<xpassword-input {{ $attributes->merge(['class' => 'component js-validation relative w-full']) }} data-uid="{{ $uid }}" data-js-initialized="false" data-attribute="{{ $attribute }}">
    <input id="js-{{ $attribute }}-input" class="input js-input" type="password" autocomplete="{{ $attribute }}" autocorrect="off" autocapitalize="off" spellcheck="false"
        inputmode="{{ $attribute }}" name="{{ $attribute }}" placeholder="" required>
