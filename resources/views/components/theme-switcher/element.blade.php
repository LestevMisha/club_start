<xtheme-switcher {{ $attributes->merge(['class' => 'component relative h-fit w-fit']) }} data-uid="{{ $uid }}" data-js-initialized="false" data-attribute="{{ $attribute }}">
    <x-loader.compiled :uid="$uid" :attribute="$attribute" size="1.5rem" borderRadius="0.5rem" />
    <div class="icon icon_2.5rem icon_grey icon_p7 icon_radius-0.5rem icon_hover-v2">
        ...
        <input id="js-{{ $attribute }}-input" type="checkbox" {{ $isLightTheme ? 'checked' : '' }} class="absolute! left-0! top-0! m-0! h-full! w-full! cursor-pointer! outline-hidden! [all:unset]">
    </div>
</xtheme-switcher>
